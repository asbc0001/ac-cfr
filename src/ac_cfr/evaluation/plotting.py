...
def plot_mccfr_reference_convergence(
    convergence_path: Path,
    summary_path: Path,
    output_path: Path,
    *,
    ...
    """Plot per-seed and median MCCFR convergence by work and training time."""
    convergence = _read_records(
        convergence_path,
    ...
    for seed in sorted({record["seed"] for record in convergence}, key=int):
        ...
        exploitability = [float(record["exploitability"]) for record in seed_records]
        iteration_axis.plot(
    ...
    ordered_summary = sorted(summary, key=lambda record: int(record["iteration"]))
    median_exploitability = [float(record["median_exploitability"]) for record in ordered_summary]
    iteration_axis.plot(
        [int(record["iteration"]) for record in ordered_summary],
    ...
    time_axis.plot(
        [float(record["median_elapsed_training_seconds"]) for record in ordered_summary],
        median_exploitability,
        color="tab:blue",
        marker="o",
        linewidth=2,
    ...
    for axis in (iteration_axis, time_axis):
        axis.axhline(
            ...
            color="tab:orange",
            linestyle="--",
            label="Validated CFR/CFR+ ceiling",
        )
        axis.set_xscale("log")
        axis.set_yscale("log")
        axis.set_ylabel("Exact exploitability (chips)")
